...
Implements the Authorization Code Grant. See [OAuth 2.0 Flows](/vi/concepts/oauth2-flows) for how this grant works conceptually, and [Config + Flow Pattern](/vi/concepts/config-flow-pattern) for the general construction shape used here.
...
See [Models](/vi/api/models) for the full interface definitions.

## Extensions

Register any combination of extensions that implement the relevant interfaces (see [Extension System](/vi/concepts/extensions)):
...
Most deployments register both — see the [full example](/vi/examples/authorization-code-pkce-oidc).
...
This makes the flow available through `srv.CreateAuthorizationResponse`, `srv.CreateConsentResponse`, and `srv.CreateTokenResponse`. See [Server](/vi/api/server).
