...
| `none` | No client secret — used for public clients (SPAs, native apps) that instead rely on [PKCE](/vi/api/pkce) to prove request integrity. |
...
- [Models](/vi/api/models) — `ClientManager` interface reference.
- [PKCE](/vi/api/pkce) — the proof mechanism public clients (`none`) use instead of a secret.
