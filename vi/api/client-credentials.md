...
Implements the Client Credentials Grant, for service-to-service calls where no end user is present. See [OAuth 2.0 Flows](/vi/concepts/oauth2-flows#client-credentials-grant-rfc-6749-4-4).
...
- [Resource Owner Password Credentials](/vi/api/ropc)
- [JWT Access Tokens](/vi/api/jwt-access-tokens) — commonly paired with Client Credentials for service identity that downstream APIs can verify without a call back to the auth server.
