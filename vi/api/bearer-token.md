...
If you need access tokens to be self-contained JWTs that resource servers can verify without a call back to the authorization server, see [JWT Access Tokens](/api/jwt-access-tokens) (RFC 9068) instead — the two are complementary: RFC 9068 defines the *token's internal structure*, RFC 6750 defines how it's *presented and typed* in the response.

## See also

- [Token Introspection](/api/introspection) — how a resource server validates an opaque bearer token.
- [JWT Access Tokens](/api/jwt-access-tokens) — a self-verifying alternative to opaque tokens.
