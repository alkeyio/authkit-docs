...
| --- | --- | --- |
| Resource server can verify without a call back | No — needs [introspection](/vi/api/introspection) | Yes — verifies the signature locally |
...
Choose JWT access tokens when you want resource servers to validate tokens without network calls to the authorization server — most useful for [Client Credentials](/vi/api/client-credentials) service-to-service tokens and high-throughput resource servers.

## See also

- [OIDC](/vi/api/oidc) — a related but distinct token: the ID Token identifies the *user*, RFC 9068 tokens authorize *API access*.
- [Token Introspection](/vi/api/introspection) — the opaque-token alternative to local verification.
