...
Introspection matters most when access tokens are opaque (see [Bearer Token](/vi/api/bearer-token)) — a resource server can't decode an opaque token itself, so it asks the authorization server. If you issue [JWT access tokens](/vi/api/jwt-access-tokens) instead, resource servers can typically verify tokens locally via signature and skip the introspection round-trip, at the cost of not being able to immediately revoke a token before it expires.

## See also

- [JWT Access Tokens](/vi/api/jwt-access-tokens) — the self-verifying alternative.
- [Custom Error Handler example](/vi/examples/custom-error-handler).
