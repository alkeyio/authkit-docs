...
This same shape applies to `ropc`, `client_credentials`, `rfc7662` (introspection), and every other flow — learn it once. See [Config + Flow Pattern](/vi/concepts/config-flow-pattern) for the reasoning behind it.
...
PKCE (`rfc7636`) and OIDC (`oidc/core/authorization_code`) are implemented as extensions and plug into the Authorization Code flow via `RegisterExtension` — a useful pattern to follow if you need to add custom claims, audience restriction, or rate limiting at the same integration points. See [Extension System](/vi/concepts/extensions).

## Models

Implement the interfaces in the `models` package with your own data layer. See [Models](/vi/api/models) for the full interface reference and `integrations/sql/` for example SQL-backed implementations.
