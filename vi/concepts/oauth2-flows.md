...
AuthKit implements four grant types from RFC 6749, plus the extensions and adjacent RFCs that make them production-ready. This page is a conceptual refresher; see [API Reference](/vi/api/authorization-code) for the exact Go types.
...
In AuthKit, this is `rfc6749/authorization_code`. It is almost always paired with [PKCE](/vi/concepts/pkce) and, if you need identity rather than just authorization, [OIDC](/vi/concepts/oidc).
...
- [RFC 6750](/vi/api/bearer-token) — how bearer tokens are formatted and presented.
- [RFC 9068](/vi/api/jwt-access-tokens) — issuing access tokens as JWTs instead of opaque strings.
- [RFC 7662](/vi/api/introspection) — letting a resource server ask the authorization server whether a token is still valid.
