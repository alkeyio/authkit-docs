...
- [PKCE](/vi/concepts/pkce) — what the built-in PKCE extension actually validates.
- [OpenID Connect](/vi/concepts/oidc) — what the OIDC extension adds to a token response.
- [Architecture](/vi/docs/architecture) — how extensions fit between `Config` and `Server`.
