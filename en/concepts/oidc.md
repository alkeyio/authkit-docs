...
AuthKit implements the OIDC Authorization Code flow's ID Token issuance as an extension (`oidc/core/authorization_code`) on top of the base Authorization Code grant, following the same pattern as [PKCE](/en/concepts/pkce) — see [Extension System](/en/concepts/extensions).
...
- [PKCE](/en/concepts/pkce) — almost always registered alongside OIDC on the same flow.
- [Authorization Code + PKCE + OIDC example](/en/examples/authorization-code-pkce-oidc) — full runnable setup.
