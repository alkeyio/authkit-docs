...
Implements the Resource Owner Password Credentials (ROPC) grant. See [OAuth 2.0 Flows](/en/concepts/oauth2-flows#resource-owner-password-credentials-rfc-6749-4-3) for when (and whether) to use this grant — it's intended for tightly-controlled, first-party clients only.
...
- [Client Credentials](/en/api/client-credentials) — for machine-to-machine calls with no user.
- [Authorization Code](/en/api/authorization-code) — the preferred grant for anything with a browser available.
